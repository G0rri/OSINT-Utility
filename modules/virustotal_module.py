...
class VirustotalModule(BaseModule):
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError("API Key de VirusTotal no encontrada. Asegúrate de configurar VT_API_KEY en tu archivo .env")

    async def run(self, target: str, callback) -> dict:
        if not self.api_key:
            callback("[-] Error crítico: API Key de VirusTotal no configurada en el archivo .env\n")
