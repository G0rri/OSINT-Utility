...
class OSINTApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.console_textbox = ctk.CTkTextbox(
            self, state="disabled", fg_color="black", text_color="#00FF00", font=("Consolas", 12)
        )
    ...
    def _on_tool_change(self):
        ...
        if choice == "Metadatos":
            ...
            self.target_entry.configure(placeholder_text="Añade la ruta del archivo (o usa el botón 📂)")
        else:
            self.btn_file.configure(state="disabled")
            if choice == "Holehe":
                self.target_entry.configure(placeholder_text="Ingresa el correo (para Holehe)")
            elif choice == "Sherlock":
                self.target_entry.configure(placeholder_text="Ingresa el username (para Sherlock)")
            elif choice == "VirusTotal":
                self.target_entry.configure(placeholder_text="Ingresa IP o Dominio (para VirusTotal)")
            elif choice == "WHOIS":
                self.target_entry.configure(placeholder_text="Ingresa el dominio (para WHOIS)")
            elif choice == "Subdominios":
                self.target_entry.configure(placeholder_text="Ingresa el dominio raíz (para Subdominios)")
            elif choice == "PortScanner":
                self.target_entry.configure(placeholder_text="Ingresa IP o Dominio (para Escáner de Puertos)")
            elif choice == "SecurityHeaders":
                self.target_entry.configure(placeholder_text="Ingresa URL o Dominio (para Cabeceras HTTP)")
            elif choice == "Wayback":
                self.target_entry.configure(placeholder_text="Ingresa URL o Dominio (para Wayback Machine)")
